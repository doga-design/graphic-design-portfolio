(function initDodollmSelection() {
  ...
  function openWithQuote({ quote, question = "" } = {}) {
    const normalizedQuote = normalizeText(String(quote || ""));
    const normalizedQuestion = normalizeText(String(question || ""));
    if (!normalizedQuote) return;

    selectionButtonPressText = "";
    hideSelectionButton();
    attachedQuote = normalizedQuote;

    // Defer until after the triggering click finishes bubbling, so the panel
    // outside-click handler does not immediately close it again.
    window.setTimeout(() => {
      window.openDodollmPanel?.();
      ...
      requestAnimationFrame(() => {
        ...
        if (normalizedQuestion) {
          input.value = normalizedQuestion;
        }
  ...
  window.askDodollmAboutQuote = openWithQuote;
})();
