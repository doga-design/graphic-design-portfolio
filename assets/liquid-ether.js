/**
 * LiquidEther — vanilla WebGL fluid background (ported from React/Three.js component).
 */
import * as THREE from "three";

const defaultColors = ["#5eb0ff", "#ff9f5a", "#2ed4c4", "#f0c840"];
...
export function initLiquidEther(container, options = {}) {
  ...
  class MouseClass {
    ...
    init(el) {
      ...
      this.listenerTarget.addEventListener("touchcancel", this._onTouchEnd);
    ...
    dispose() {
      if (this.listenerTarget) {
        ...
        this.listenerTarget.removeEventListener("touchcancel", this._onTouchEnd);
      }
      if (this.docTarget) {
  ...
  class WebGLManager {
    constructor(props) {
      ...
      Mouse.onHoldChange = props.onHoldChange ?? null;
      Mouse.onInteract = () => {
        this.lastUserInteraction = performance.now();
  ...
  const ro = new ResizeObserver(() => {
    ...
    resizeRafRef = requestAnimationFrame(() => webglRef?.resize());
  });
